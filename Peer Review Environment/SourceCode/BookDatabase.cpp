...
BookDatabase::BookDatabase( const std::string & filename )
{
  ...
  Book tmp;
  while(fin >> tmp){
    this->_data.insert(std::pair<std::string, Book>(tmp.isbn(),tmp));
  }
  /////////////////////// END-TO-DO (2) ////////////////////////////
...
Book* BookDatabase::find( const std::string & isbn ){
  std::map<std::string, Book>::iterator iter = this->_data.find(isbn);

  if(iter == this->_data.end()) return nullptr;
